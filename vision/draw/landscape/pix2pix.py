...
parser.add_argument("--inference", type=int, default=0, help="inference mode")
...
generator = UnetGenerator(3, 3, 7, 64, norm_layer=nn.BatchNorm2d, use_dropout=True, use_noise=True)
...
class ResizeFilp:
    def __init__(self, size, mode, p=0.5):
        self.size = size
        self.mode = mode
        self.p = p
        self.normal = transform.ImageNormalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
        
    def __call__(self, imgA:Image.Image, imgB:Image.Image):
        if not isinstance(imgA, Image.Image):
            imgA = transform.to_pil_image(imgA)
        if not isinstance(imgB, Image.Image):
            imgB = transform.to_pil_image(imgB)
        imgA = transform.resize(imgA, self.size, self.mode)
        imgB = transform.resize(imgB, self.size, self.mode)
        if random.random() < self.p:
            imgA = imgA.transpose(Image.FLIP_LEFT_RIGHT)
            imgB = imgB.transpose(Image.FLIP_LEFT_RIGHT)
        if random.random() < self.p:
            imgA = transform.vflip(imgA)
            imgB = transform.vflip(imgB)
        imgA = transform.to_tensor(imgA)
        imgB = transform.to_tensor(imgB)
        imgA = self.normal(imgA)
        imgB = self.normal(imgB)
        return imgA, imgB

# Configure dataloaders
# 数据增强
# transforms = ResizeFilp(size=(opt.img_height, opt.img_width), mode=Image.BICUBIC)
# 无数据增强
transforms = ResizeFilp(size=(opt.img_height, opt.img_width), mode=Image.BICUBIC, p=0)
...
val_dataloader = ImageDataset(opt.data_path, mode="val", transforms=val_transforms).set_attrs(
    batch_size=opt.batch_size,
    shuffle=False,
    num_workers=1,
)
...
def eval(epoch, writer):
    ...
    for i, (_, real_A, photo_id) in enumerate(val_dataloader):
        ...
        for idx in range(fake_B.shape[0]):
            img_data = fake_B[idx].transpose(1,2,0)[:,:,::-1]
            # print(photo_id[idx])
            cv2.imwrite(f"{opt.output_path}/images/test_fake_imgs/epoch_{epoch}/{photo_id[idx]}.jpg", img_data)
            cnt += 1
...
if __name__ == "__main__":
    generator.load(os.path.join(f"{opt.output_path}/saved_models/generator_140.hzb5.pkl"))
    if opt.inference:
